...
namespace Autowatchers.Net7
{
    public class WithBlockNamespace : IDisposable
    {
        ...
        {
            _logger.LogInformation("Se ha escrito la propiedad Name: Nuevo valor: {NewValue} y antiguo valor: {OldValue}.", newValue, oldValue);
        }

        public void EmailFunc(string oldValue, string newValue)
        {
            _logger.LogInformation("Se ha escrito la propiedad Email: Nuevo valor: {NewValue} y antiguo valor: {OldValue}.", newValue, oldValue);
        }

        public void PhoneNumberFunc(string oldValue, string newValue)
        {
            _logger.LogInformation("Se ha escrito la propiedad PhoneNumber: Nuevo valor: {NewValue} y antiguo valor: {OldValue}.", newValue, oldValue);
        }
